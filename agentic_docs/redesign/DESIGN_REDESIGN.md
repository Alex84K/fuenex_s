...
- **Лендинг и страница приватности** — исходно свой визуальный мир с захардкоженной синей
  палитрой (`index.css`, ~1150 строк). **17 августа 2026, уже после перекраски, лендинг
  переведён на палитру дизайн-системы** (кирпичный акцент `#A8412E`, тёплые поверхности,
  navy-плашки) и переименован с FUENEX на Stroylit — см. `DESIGN §8`. При этом он остаётся
  светлым островом: фон задан явно, `color-scheme: light`, переключение темы его не затрагивает.
...
- **не трогает лендинг** (D13) — исходное решение. **Изменено 17 августа 2026 вне рамок
  перекраски, по прямому запросу:** лендинг переведён на палитру дизайн-системы и имя Stroylit
  (`index.css` полностью перекрашен, `LandingPage.tsx`/`PrivacyPage.tsx` переименованы,
  фавикон — кирпичный `#A8412E`). Лендинг по-прежнему не участвует в тёмной теме;
